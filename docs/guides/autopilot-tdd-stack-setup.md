...
Copy the following files into your new repo (paths are relative to repo root). These come directly from the hardened DroidForge setup:

```
AGENTS.md
scripts/start-agent-work.sh
scripts/autopilot-reset.sh
scripts/autopilot-wrapup.sh
docs/guides/taskmaster-guardrails.md
docs/guides/human-in-the-loop-workflow.md
docs/guides/autopilot-tdd-playbook.md (optional, but recommended if available)
docs/project/onboarding-flow-todo.md (for logging guardrail work)
.windsurf/rules/taskmaster.md (if using Windsurf)
```

> Keep the directory structure (`docs/guides`, `scripts/`, etc.) when copying.
...
> **Autopilot paused:** it spotted files you changed that aren’t saved into Git yet. Run the commands below, confirm `git status -sb` shows nothing left to save, then rerun the helper.
...
If any file still refuses to stage, check whether it is ignored:

```bash
git check-ignore -v <path>
```

Once `git status -sb` reports no pending changes, rerun `./scripts/start-agent-work.sh`.
