...
1. Read `AGENTS.md` and skim the relevant Windsurf rules (`.windsurf/rules/taskmaster.md`, `.windsurf/rules/dev_workflow.md`).
2. Run:
   ```bash
   task-master list --with-subtasks
   ```
   Do **not** open editors or touch code until the live task tree has been reviewed.
...
- Proposed tags follow the patterns in `.windsurf/rules/dev_workflow.md` (branch mirroring, experiments, etc.) and **must** be acknowledged by the user before execution.
