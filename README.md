...
- **Interactive droid sessions**: `~/.factory/sessions/<session-id>.jsonl` (matching `.settings.json` files hold metadata). Each JSON line captures the dialogue and tool calls.
- **UAT automation runs**: `~/.factory/uat-test-logs/uat2-transcript-<timestamp>.log` (generated by the Expect scripts in `scripts/`).
